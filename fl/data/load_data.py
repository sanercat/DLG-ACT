...
def load_data(args, dataset=None, batch_size=None):
    ...
    if dataset == 'MNIST':
        ...
        data_transform = transforms.Compose([
                         transforms.ToTensor(),
                         transforms.Normalize(data_mean, data_std)])
        trainset = CIFAR100(root=args.root, train=True, download=False,
                           transform=data_transform)
        testset = CIFAR100(root=args.root, train=False, download=False,
                          transform=data_transform)
        total_data = len(trainset.data)
        if args.defense == 'ats':
            trainset.transform = comm.build_transform(
        ...
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])
        trainset = ImageFolder(root=args.root + '/tiny-imagenet-200/train',
                               transform=transforms.Compose([
                                          transforms.Resize(256),
                                          transforms.CenterCrop(224),
                                          transforms.ToTensor(),
                                          normalize,]))
        testset = ImageFolder(root=args.root + '/tiny-imagenet-200/val',
                              transform=transforms.Compose([
                                          transforms.Resize(256),
                                          transforms.CenterCrop(224),
                                          transforms.ToTensor(),
    ...
    if args.method == 'iid':
        ...
        train_y =np.array([trainset.targets[idx] for idx in trainset_sens.indices])
