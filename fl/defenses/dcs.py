...
def defense_optim(args, model, loss_fn, gt_imgs, gt_labels, dm, ds, device, proxy_imgs, proxy_labels, save_path):
    ...
    loss = ((args.lambda_y * loss_fn(adv_out[:args.num_sen * args.per_adv], proxy_labels.to(device)) 
         + (1 - args.lambda_y) * loss_fn(adv_out[:args.num_sen * args.per_adv], sen_label.to(device))) 
        + loss_fn(adv_out[args.num_sen * args.per_adv:], gt_labels.to(device))) / divd
